...
- Image generation: OpenAI DALL·E 3 (`openai`) or Google Gemini image models via `google-genai` / Vertex AI.
...
│   └── image_generator.py        # Prompt → PNG via DALL·E or Gemini (+ aspect-ratio helpers)
...
├── synth-dataset-notebook.ipynb  # Kaggle runner: clone, install, set secrets, `python -m pipeline`
...
StructuredRadiologyPrompt
    ...
    ↓  image_generator.generate_image             → DALL·E 3 or Gemini image API
...
flowchart TD
    ...
    FMT -- prompt text --> IG["image_generator"]
    ORCH -- ref image paths + source_dimensions --> IG
    IG --> OUT["output/images/*.png"]
...
|---|---|
| **config** (`pipeline/config.py`) | Loads `../.env` via `dotenv`, exposes keys/model ids/paths as module constants, detects Kaggle (`IS_KAGGLE`), holds `GEMINI_SUPPORTED_ASPECT_RATIOS` / `DALLE_SUPPORTED_SIZES`, and creates `IMAGES_DIR` at import. Depends on nothing internal; imported by everything else. |
...
- **Models:** `CHAT_MODEL` (default `gemini-2.0-flash`), `IMAGE_GENERATOR` (`dalle`|`gemini`), `GEMINI_IMAGE_MODEL`, `DALLE_MODEL`, `DALLE_IMAGE_SIZE`, `DALLE_IMAGE_QUALITY`.
...
CLI flags (`--generator`, `--limit`, `--offset`, `--csv`, `--projections-csv`, `--images-dir`, `--skip-images`) override config at call time; the pattern is `value = arg or config.DEFAULT`.
...
8. **Calling the image API:** never call OpenAI/Gemini SDKs directly from a new module. Route through `image_generator.generate_image(prompt, uid, generator=..., image_paths=..., view_suffix=..., source_dimensions=...)`, which returns a `Path`. To add a *new image back-end*, write `generate_image_<name>(...)` in `image_generator.py` mirroring the existing signature (prompt, uid, view_suffix, source_dimensions, `max_retries=3` with `2**attempt` backoff, save PNG to `config.IMAGES_DIR`, return the `Path`), add a branch to the `generate_image` dispatcher, and extend the `--generator` argparse `choices` list.
...
- **Defer provider SDK imports** into functions (`from openai import OpenAI`, `from google import genai`, `from langchain_openai import ChatOpenAI`). Top-level imports would make the package unimportable when only one provider's dependencies are installed — `test_pipeline.py` relies on this.
...
- **Reference images are Gemini-only.** DALL·E ignores `image_paths` with a warning. `--images-dir` also requires `--projections-csv`, or reference images are silently disabled.
...
- **Aspect ratios** come from the tables in `config.py`; extend those dicts rather than hardcoding ratio strings. DALL·E 3 supports only its three sizes.
